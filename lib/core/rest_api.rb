...
def post_method(action, body)
  ...
  payload = nil
  if body["Multipart"]
    file_m = convert_value(body["File"])
    file_m = File.join(convert_value(body["Folder"]), file_m) if body["Folder"]
    if !File.exist? file_m
      folder = (body["Folder"] ? convert_value(body["Folder"]) : ".")
      Dir.entries(folder).select {
        ...
        if file.match(/#{convert_value(body["File"])}/)[0]
          ...
          file_m = File.join(folder, file_m)
          break
        end
      end
    end
